 ...
 * @param {function} cb Call back function.
 */
userDao.createUser = function (username, password, from, temporary, cb){
	var sql = 'insert into User (name,password,`from`,temporary,loginCount,lastLoginTime) values(?,?,?,?,?,?)';
	var loginTime = Date.now();
	var args = [username, password, from || '', temporary, 1, loginTime];
	pomelo.app.get('dbclient').insert(sql, args, function(err,res){
		if(err !== null){
			utils.invokeCallback(cb, {code: err.number, msg: err.message}, null);
		} else {
			var user = new User({id: res.insertId, name: username, password: password, temporary: temporary, loginCount: 1, lastLoginTime:loginTime});
			utils.invokeCallback(cb, null, user);
		}
	});
};
